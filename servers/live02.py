...
fig = figure(x_axis_type="datetime", tooltips=[("Accel", "@Accel")], title = "Acceleration (mg)", sizing_mode='stretch_both')
...
def update_chart():
    ...
    global last_index
    with lock:
        abs_accel = np.mean(np.array(data_accel[last_index+1:]), axis=-1) - 980/3.0
